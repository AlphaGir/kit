	...
	case LEX_NORMAL:
		{
			...
			{
			p = get_tkstr(token);
			printf("\e[33m%s\033[0m",p);
			}
			else if(token >= KW_CHAR)
				//SetConsoleTextAttribute(h,FOREGROUND_GREEN|FOREGROUND_INTENSITY);
			{
			p = get_tkstr(token);
                        printf("\e[32m%s\033[0m",p);
                        }
			else if(token >= TK_CINT)
				//SetConsoleTextAttribute(h,FOREGROUND_RED|FOREGROUND_GREEN);           
			{
			p = get_tkstr(token);
                        printf("\e[37m%s\033[0m",p);
                        }
			else 
				//SetConsoleTextAttribute(h,FOREGROUND_RED|FOREGROUND_INTENSITY);		
			{
			p = get_tkstr(token);
		        printf("\e[31m%s\033[0m",p);
			}
			break;
		}
	case LEX_SEP:
...
void skip_white_space()
{
	...
	{			
		...
		printf("%c",ch); //这句话，决定是否打印空格，如果不输出空格，源码中空格将被去掉，所有源码挤在一起 喔 挤在一起会怎么样呢???
		getch();
	}
}
